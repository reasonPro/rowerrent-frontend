    ...
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-white shadow-sm border-b border-gray-100">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden xl:flex items-center gap-4">
          ...
          {!isLoggedIn ? (
            <>
              <Button variant="ghost" size="sm" onClick={onLoginClick}>{t("header.login") || "Вхід"}</Button>
              <Button size="sm" onClick={onRegisterClick} className="bg-green-600 hover:bg-green-700 text-white">{t("header.signup") || "Реєстрація"}</Button>
            </>
          ) : (
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="xl:hidden overflow-hidden border-t border-gray-100 bg-white shadow-lg max-h-[80vh] overflow-y-auto">
            <div className="p-4 space-y-4">
              ...
              <div className="py-2 border-b border-gray-50">
                 <p className="text-xs text-gray-500 mb-1 font-medium uppercase tracking-wider">{t("header.openingHours") || "Час роботи"}</p>
              ...
              {!isLoggedIn ? (
                <div className="flex flex-col gap-3 pt-2">
                  <Button variant="outline" className="w-full" onClick={() => { onLoginClick?.(); setIsMenuOpen(false); }}>{t("header.login") || "Вхід"}</Button>
                  <Button className="w-full bg-green-600 text-white" onClick={() => { onRegisterClick(); setIsMenuOpen(false); }}>{t("header.signup") || "Реєстрація"}</Button>
                </div>
              ) : (
